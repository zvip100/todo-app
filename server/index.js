...
app.post('/tasks', async (req, res) => {
    const result = await db.one('insert into public.task (title, user_id) values (${title}, ${user_id}) returning *', {
...
app.get('/tasks', async (req, res) => {
    const result = await db.many('select * from public.task where deleted_at is null');
    res.json(result.map(task => ({ id: task.id, title: task.title, done: task.status !== 'active'})))
})


app.patch('/tasks/:id', async (req, res) => {
    const result = await db.none("update public.task set status = 'done' where id = ${id}", {
        id: req.params.id
    })
    res.json({ok: true})
})
